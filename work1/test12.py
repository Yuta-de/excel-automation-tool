...
def gui_main():
    ...
    tk.Button(root, text="選択", command=lambda: input_var.set(filedialog.askdirectory())).grid(row=0, column=2)

    # テンプレート
    tk.Label(root, text="テンプレート").grid(row=1, column=0)
    template_var = tk.StringVar()
    tk.Entry(root, textvariable=template_var, width=50).grid(row=1, column=1)
    tk.Button(root, text="選択", command=lambda: template_var.set(filedialog.askopenfilename())).grid(row=1, column=2)

    # 出力ファイル
    tk.Label(root, text="出力ファイル").grid(row=2, column=0)
    output_var = tk.StringVar()
    tk.Entry(root, textvariable=output_var, width=50).grid(row=2, column=1)
    tk.Button(root, text="選択", command=lambda: output_var.set(filedialog.asksaveasfilename())).grid(row=2, column=2)
    ...
    def run_main():
        config["PATH"]["input_folder"] = input_var.get() + "\\*.xlsx"
        config["PATH"]["template_path"] = template_var.get()
        config["PATH"]["output_path"] = output_var.get()
        threading.Thread(target=main, daemon=True).start()

    tk.Button(root, text="実行", command=run_main).grid(row=3, column=1)
